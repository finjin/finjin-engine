...
PVRReader::ReadHeaderResult PVRReader::ReadHeader(ByteBufferReader& reader)
{
    if (reader.Read(&this->header, sizeof(Header)) < sizeof(Header))
        return ReadHeaderResult::FAILED_TO_READ_HEADER;
    
    if (this->header.version != 0x03525650 && this->header.version != 0x50565203)
        return ReadHeaderResult::INVALID_HEADER_VERSION;
    
    auto swapBytes = this->header.version == 0x50565203;
    if (swapBytes)
    ...
    {
        uint32_t fourCC;
        if (reader.ReadAndSwap(&fourCC, sizeof(fourCC), swapBytes) != sizeof(fourCC))
            return ReadHeaderResult::BAD_METADATA_FOURCC;
        metadataSizeLeft -= sizeof(fourCC);
        
        uint32_t key;
        if (reader.ReadAndSwap(&key, sizeof(key), swapBytes) != sizeof(key))
            return ReadHeaderResult::BAD_METADATA_KEY;
        metadataSizeLeft -= sizeof(key);
        
        uint32_t dataSize;
        if (reader.ReadAndSwap(&dataSize, sizeof(dataSize), swapBytes) != sizeof(dataSize))
                ...
                switch (key)
                {
                    case 0:
                    {
                        while (dataSize > 0)
                        {
                            ...
                            reader.ReadAndSwap(&x, sizeof(x), swapBytes);
                            reader.ReadAndSwap(&y, sizeof(y), swapBytes);
                            reader.ReadAndSwap(&width, sizeof(width), swapBytes);
                            if (reader.ReadAndSwap(&height, sizeof(height), swapBytes) < sizeof(height))
                    ...
                    case 1:
                    {
                        reader.ReadAndSwap(&this->headerExtension.normalMapScale, sizeof(this->headerExtension.normalMapScale), swapBytes);
                    ...
                    case 4:
                    {
                        ...
                        {
                            if (reader.ReadAndSwap(&this->headerExtension.border[i], sizeof(this->headerExtension.border[i]), swapBytes) < sizeof(this->headerExtension.border[i]))
...
Utf8String PVRReader::GetReadHeaderResultString(ReadHeaderResult result) const
{
    switch (result)
    {
        ...
        case ReadHeaderResult::INVALID_HEADER_VERSION: return "Invalid header version.";
...
void PVRReader::ReadHeader(ByteBufferReader& reader, Error& error)
{
...
void PVRReader::ReadMipLevels(ByteBufferReader& reader, MipLevelFunction mipLevelFunction, Error& error)
{
    FINJIN_ERROR_METHOD_START(error);
    
    if (mipLevelFunction == nullptr)
    {
    ...
    if (!this->header.IsCompressed())
    {
        ...
        for (uint32_t mipLevel = 0; mipLevel < 1 + this->header.mipMapCount; mipLevel++)
        {
            ...
            for (uint32_t surfaceIndex = 0; surfaceIndex < this->header.surfaceCount; surfaceIndex++)
            {
                ...
                {
                    for (uint32_t depthIndex = 0; depthIndex < this->header.depth; depthIndex++)
                    {
                        if (!mipLevelFunction(this, mipLevel, surfaceIndex, faceIndex, depthIndex, width, height, imageSizeBytes, pixels))
                            break;
