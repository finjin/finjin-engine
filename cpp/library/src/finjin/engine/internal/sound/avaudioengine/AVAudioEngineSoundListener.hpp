...
namespace Finjin { namespace Engine {
    ...
    //Manages access to the device context's one and only listener. Many sound listeners can be created, but only one can be Select()ed at a time.
    class AVAudioEngineSoundListener
    {
    public:
        AVAudioEngineSoundListener();
        ~AVAudioEngineSoundListener();
