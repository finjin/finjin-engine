...
XcbConnection::CreateResult XcbConnection::Create(const Utf8String& displayName)
{
    ...
    if (xcb_connection_has_error(this->c))
    {
        ...
        return CreateResult::FAILED_TO_CONNECT_TO_DISPLAY_SERVER;
    }
    ...
    if (!xcb_ewmh_init_atoms_replies(&this->ewmh, ewmhCookie, nullptr))
    {
        ...
        return CreateResult::FAILED_TO_INITIALIZE_XCB_DATA;
    }
...
void XcbConnection::Create(const Utf8String& displayName, Error& error)
{
    ...
    switch (Create(displayName))
    {
        case CreateResult::FAILED_TO_CONNECT_TO_DISPLAY_SERVER:
        {
            FINJIN_SET_ERROR(error, "Failed to connect to the display server.");
            return;
        }
        case CreateResult::FAILED_TO_INITIALIZE_XCB_DATA:
        {
...
void XcbConnection::Destroy()
{
    if (this->c != nullptr)
    {
        ...
        FINJIN_ZERO_MEMORY(&this->ewmh, sizeof(this->ewmh));
...
void XcbConnection::AddWindow(xcb_window_t window, XcbWindow* windowPointer, Error& error)
{
    FINJIN_ERROR_METHOD_START(error);

    if (this->windowLookup.insert(window, windowPointer, false).HasErrorOrValue(false))
        FINJIN_SET_ERROR(error, "Failed to add window to connection's window lookup table.");
}
...
bool XcbConnection::GetDefaultDisplayName(Utf8String& result)
{
    result.clear();
...
void XcbConnection::GetDefaultDisplayName(Utf8String& result, Error& error)
{
    ...
    if (!GetDefaultDisplayName(result))
        FINJIN_SET_ERROR(error, "Failed to assign default display name or it was empty.");
}

XcbConnection::GetOrCreateResult XcbConnection::GetOrCreate(std::shared_ptr<XcbConnection>& connection, const Utf8String& requestedDisplayName)
{
    ...
    {
        if (!GetDefaultDisplayName(defaultDisplayName))
            return GetOrCreateResult::FAILED_TO_GET_DEFAULT_DISPLAY_NAME;
    ...
    if (connection == nullptr)
    {
        ...
        if (connection->Create(*displayName) != XcbConnection::CreateResult::SUCCESS)
        {
            connection.reset();
            return GetOrCreateResult::FAILED_TO_CREATE_CONNECTION;
        }
        else
            connections.insert(*displayName, connection, false);
    }

    return GetOrCreateResult::SUCCESS;
}
...
std::shared_ptr<XcbConnection> XcbConnection::GetOrCreate(const Utf8String& requestedDisplayName, Error& error)
{
    ...
    switch (GetOrCreate(connection, requestedDisplayName))
    {
        case GetOrCreateResult::FAILED_TO_GET_DEFAULT_DISPLAY_NAME:
        {
            FINJIN_SET_ERROR(error, "Failed to get default display name.");
            return nullptr;
        }
        case GetOrCreateResult::FAILED_TO_CREATE_CONNECTION:
        {
