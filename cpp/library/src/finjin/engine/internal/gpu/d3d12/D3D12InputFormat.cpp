...
void D3D12InputFormat::Create(const GpuInputFormatStruct& inputFormat, Allocator* allocator, Error& error)
{
    ...
    {
        FINJIN_SET_ERROR(error, "Failed to create D3D input layout collection.");
        return;
    }
    ...
    for (size_t i = 0; i < inputFormat.elements.size(); i++)
    {
        ...
        switch (element.type)
        {
            ...
            case NumericStructElementType::FLOAT3x3: FINJIN_SET_ERROR(error, "Failed to use 'float3x3' as vertex component."); return; 
            case NumericStructElementType::FLOAT4x3: FINJIN_SET_ERROR(error, "Failed to use 'float4x3' as vertex component."); return;
            case NumericStructElementType::FLOAT4x4: FINJIN_SET_ERROR(error, "Failed to use 'float4x4' as vertex component."); return;
            case NumericStructElementType::NESTED_STRUCT: FINJIN_SET_ERROR(error, "Failed to use 'nested struct' as vertex component."); return;
            default: FINJIN_SET_ERROR(error, "Unrecognized vertex component."); return;
        }
    }
}
